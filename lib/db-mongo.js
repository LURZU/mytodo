...
function DB(credentials) {
  ...
  self.init = () => {
    return new Promise((resolve, reject) => {
      // Utiliser directement l'URI MongoDB si disponible
      let connectionString = process.env.MONGO_URI || process.env.MONGODB_URI;

      // Si l'URI directe n'est pas disponible, alors construire à partir des variables individuelles
      ...
      // Si aucune connexion valide n'est disponible, échouer
      if (!connectionString) {
        reject(new Error("Aucune information de connexion MongoDB valide n'a été trouvée"));
        return;
      }
